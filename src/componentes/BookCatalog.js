...
const BookCatalog = () => {
  ...
  // 👤 Obtener datos del usuario del localStorage/sessionStorage
  const userData = JSON.parse(
    localStorage.getItem('userData') || 
    sessionStorage.getItem('userData') || 
    '{"nombre": "Usuario", "matricula": "N/A"}'
  );
  ...
  const handleReserveBook = (book) => {
    if (book.estatus?.toLowerCase() !== 'disponible') return;
    
    console.log('📖 Reservando libro:', book.titulo);
    
    // Navegar al formulario de préstamo con el libro preseleccionado
    navigate('/prestamo', { 
      state: { 
        preselectedBook: {
          id: book.id_libro || book.id,
          titulo: book.titulo,
          autor: book.autor,
