...
const BookRegistration = () => {
  ...
  const handlePdfFilesChange = (e) => {
    ...
    if (oversizedFiles.length > 0) {
      setErrors((prev) => ({
        ...prev,
        pdfFiles: "Cada archivo PDF no debe exceder 2GB",
      }));
      return;
    }
    ...
    if (totalSize > 2 * 1024 * 1024 * 1024) {
      setErrors((prev) => ({
        ...prev,
        pdfFiles: "El tamaño total de los archivos PDF no debe exceder 2GB",
      }));
      return;
    }
  ...
  const handleCoverImageChange = (e) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setErrors((prev) => ({
        ...prev,
        coverImage: "La imagen no debe exceder 5MB",
      }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    submitData.append(
      "ejemplares",
      formData.tipo === "digital" ? "0" : formData.ejemplares
    );
  ...
  // Función para navegar al historial de préstamos <-- AGREGADO
  const handleGoToHistory = () => {
    navigate("/historial-prestamos");
  };
      ...
      <div className="book-registration">
        <div className="registration-container">
          <div className="registration-header">
            {" "}
            {/* <-- CONTENEDOR DEL H1 CON FONDO AZUL */}
            <h1>Registro de Libros</h1>
          </div>
          
          {/* INICIO DE CAMBIO: NUEVO CONTENEDOR PARA CENTRAR EL BOTÓN */}
          <div className="history-btn-wrapper">
            <button
              type="button"
              className="history-btn" // <-- CLASE AGREGADA PARA ESTILO
